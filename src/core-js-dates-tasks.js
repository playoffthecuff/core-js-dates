...
function getTime(date) {
  const formatTime = (input) => input.toString().padStart(2, '0');
...
function getCountWeekendsInMonth(month, year) {
  ...
  while (date.getMonth() === month - 1) {
    if (date.getDay() === 6 || date.getDay() === 0) counter += 1;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const start = new Date(...parse(period.start));
  const end = new Date(...parse(period.end));
  function format(date) {
    return `${date.getDate().toString().padStart(2, '0')}-${(date.getMonth() + 1).toString().padStart(2, '0')}-${date.getFullYear()}`;
  }
  while (start <= end) {
    for (let i = 0; i < countWorkDays; i += 1) {
      if (start <= end) result.push(format(start));
      start.setDate(start.getDate() + 1);
    }
